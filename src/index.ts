...
import { Data, LoggingLevel } from "./types.js";
import { EventFilter } from "./unifier/api/eventNotifications/types.js";
...
async function main() {
  ...
  const baseURL: string = args["url"] || settings["unifier"]["url"];
  const userToken: string = args["token"] || settings["unifier"]["token"];
  const dbFile: string =
    args["dbFile"] || settings["settings"]["dbFile"] || `${process.cwd()}/unifier-db.json`;
  const frequency: number =  args["frequency"]|| settings["settings"]["frequency"];
  const loggingLevel: LoggingLevel = args["loggingLevel"] || settings["settings"]["loggingLevel"];
